...
BYTE getUSBUSART(BYTE* buffer, BYTE len)
{
	static BYTE i;
	static BYTE rx_bytes_read = 0;
    ...
    if(!USBHandleBusy(CDCDataOutHandle))
    {
         ...
         * the actual number of BYTEs received.
         */
        if(len > USBHandleGetLength(CDCDataOutHandle) - rx_bytes_read)
            len = USBHandleGetLength(CDCDataOutHandle) - rx_bytes_read;
        
        /*
         * Copy data from dual-ram buffer to user's buffer
         */
		for (i = 0; i < len; i++)
			buffer[i] = cdc_data_rx[rx_bytes_read++]; 

        /*
         * Prepare dual-ram buffer for next OUT transaction
         */
		if (rx_bytes_read >= USBHandleGetLength(CDCDataOutHandle))
		{
        	CDCDataOutHandle = USBRxOnePacket(CDC_DATA_EP,(BYTE*)&cdc_data_rx,sizeof(cdc_data_rx));
			rx_bytes_read = 0;
		}
